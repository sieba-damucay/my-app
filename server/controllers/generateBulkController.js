...
async function insertStudents(students, teacherId, res) {
  ...
  for (const student of students) {
    ...
    try {
      ...
      // 2. Insert student
      const sqlInsert = `
        INSERT INTO users (name, username, password_hash, grade_level, section_id, role, teacher_id)
        VALUES (?, ?, ?, ?, ?, 'student', ?)
      `;
      ...
      await new Promise((resolve, reject) => {
        db.query(
          sqlInsert,
          [Name, Username, password_hash, Grade, section_id, teacherId],
          (err) => {
            if (err) {
  ...
  try {
    if (fileExt === "csv") {
      fs.createReadStream(req.file.path)
        ...
        .on("end", async () =>
          generateQRsAndSave(students, res, req.file.path)
        );
...
async function generateQRsAndSave(students, res, uploadedFilePath) {
  ...
  for (const student of students) {
    ...
    try {
      ...
      const sqlInsert = `INSERT INTO users (name, username, password_hash, grade_level, section_id, role)VALUES (?, ?, ?, ?, ?, 'student')`;

      await new Promise((resolve, reject) => {
        db.query(
          sqlInsert,
          [Name, username, password_hash, Grade, section_id],
          (err) => {
            if (err) reject(err);
            else resolve();
          }
  ...
  for (const student of students) {
    ...
    const Grade = student.Grade || student.grade;
    const baseUsername =
      Name.toLowerCase().replace(/\s+/g, ".") + Grade.replace("Grade ", "");
