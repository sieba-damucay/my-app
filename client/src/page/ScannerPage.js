...
function ScannerPage() {
  ...
  const handleError = (err) => {
    ...
    if (err.name === "NotAllowedError") {
      setMsg({ text: "Camera permission denied. Please allow permissions.", type: "error" });
    } else if (err.name === "NotReadableError") {
      setMsg({ text: "Camera already in use by another application.", type: "error" });
    } else {
      setMsg({ text: "Camera error. Please check your device.", type: "error" });
    }
  };
    ...
    >
      ...
      >
        ...
        <p className="text-white mb-4 text-center" style={{ fontSize: "0.9rem", opacity: 0.85 }}>
          ...
          in front of your camera. Hold your device steady for accurate scanning.
        </p>
        ...
        >
          <div
            style={{
              ...
              width: "200%",
              height: "200%",
              background: "conic-gradient(maroon, gold, maroon, gold, maroon)",
          ...
          >
            {cameraAllowed ? (
              ...
              >
                {msg.text || "Camera not accessible. Please allow permissions."}
              </div>
            )}
          </div>
        </div>
